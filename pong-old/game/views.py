...
def update_game(request):
    if request.method == 'POST':
        try:
            ...
            game_state.player1_y = max(0, min(300, data.get('player1_y', game_state.player1_y)))
            game_state.player2_y = max(0, min(300, data.get('player2_y', game_state.player2_y)))
            game_state.save()
        ...
        try:
            ...
            if (game_state.ball_x <= 20 and game_state.player1_y <= game_state.ball_y <= game_state.player1_y + 100) or \
               (game_state.ball_x >= 780 and game_state.player2_y <= game_state.ball_y <= game_state.player2_y + 100):
