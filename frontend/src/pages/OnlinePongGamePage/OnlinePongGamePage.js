   ...
   sock.on('game_state_update', (data) => {
	 try {
	   ...
	   setGamePositions((prevPositions) => ({
		 ...prevPositions,
		 player1Position: data.player_1_position * scaleFactor,
		 player2Position: data.player_2_position * scaleFactor,
		 ball: {
		   x: data.ball_x_position * scaleFactor,
		   y: data.ball_y_position * scaleFactor,
		 },
		 ballDirection: {
		   x: data.ball_x_direction * scaleFactor,
		   y: data.ball_y_direction * scaleFactor,
		 },
	   }));
	   setGameDimensions((prevDimensions) => ({
		 ...prevDimensions,
		 game: {
		   width: data.game_width * scaleFactor,
		   height: data.game_height * scaleFactor,
		 },
		 paddle: {
		   width: data.paddle_width * scaleFactor,
		   height: data.paddle_height * scaleFactor,
		   offset: data.paddle_offset * scaleFactor,
		 },
		 ball: {
		   radius: data.ball_radius * scaleFactor,
		 },
	   }));
	   const currentScore = gameScore();
	   if (
		 data.player_1_score !== currentScore.player1.score ||
		 data.player_2_score !== currentScore.player2.score
	   ) {
		 setGameScore((prevScore) => ({
		   ...prevScore,
		   player1: { score: data.player_1_score },
		   player2: { score: data.player_2_score },
		 }));
	   }
	 } catch (error) {
